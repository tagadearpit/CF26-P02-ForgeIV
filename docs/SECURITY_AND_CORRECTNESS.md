...
| Workflow data isolation | The execution list, dashboard counts, and execution details are scoped to the requester; only `ADMIN` may view every workflow. | Prevents one self-registered account from reading another account’s workflow history. |
| Configured second administrator | An optional paired Render configuration creates one additional `ADMIN` using a bcrypt hash; no configured credential is kept in source. | Supports a real team administrator without leaking its password into GitHub or the browser. |
