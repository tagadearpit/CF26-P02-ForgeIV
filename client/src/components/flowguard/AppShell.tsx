/** FlowGuard Calm Operations Console: an anchored sidebar shell where restrained motion explains route changes and live state without decoration. */
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <SidebarProvider defaultOpen>
    <Sidebar variant="inset" collapsible="icon" className="border-r border-slate-200/70 bg-[#FBFCFE]">
      ...
      <SidebarContent className="px-2 pt-2">
        ...
        <Link href="/tour" className="fg-interactive mx-3 mt-4 block rounded-xl bg-slate-900 p-3 text-white no-underline hover:-translate-y-0.5 group-data-[collapsible=icon]:hidden">
      ...
      <SidebarFooter className="p-3">
        <button onClick={() => setProfileOpen(value => !value)} className="fg-interactive flex w-full items-center gap-2 rounded-lg p-2 text-left hover:bg-slate-100">
        ...
        {profileOpen && <div className="fg-surface-enter mt-1 rounded-lg border border-slate-200 bg-white p-1.5 shadow-lg group-data-[collapsible=icon]:hidden"><button onClick={signOut} className="fg-interactive w-full rounded-md px-2 py-1.5 text-left text-xs font-semibold text-rose-700 hover:bg-rose-50">Sign out</button></div>}
      </SidebarFooter>
    </Sidebar>
    <SidebarInset className="min-h-svh bg-[#F5F7FB]">
      <header className="flex h-[70px] shrink-0 items-center justify-between border-b border-slate-200/80 bg-[#F9FAFC]/90 px-4 backdrop-blur-xl sm:px-7">
        ...
        <div className="flex items-center gap-2"><div className="hidden items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-500 sm:flex"><span className="fg-health-signal size-1.5 rounded-full bg-emerald-500" />All participants healthy</div><Button variant="ghost" size="icon" className="fg-interactive relative text-slate-500"><Bell className="size-4" /><span className="absolute right-2 top-2 size-1.5 rounded-full bg-amber-400" /></Button><Button asChild variant="ghost" size="icon" className="fg-interactive text-slate-500"><Link href="/tour" aria-label="Open judge tour"><CircleHelp className="size-4" /></Link></Button></div>
      </header>
