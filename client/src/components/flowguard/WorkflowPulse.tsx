/** FlowGuard Calm Operations Console: the workflow pulse rail uses small live-state cues to make active, waiting, and recovery work readable at a glance. */
...
export function WorkflowPulse({ steps, compact = false }: { steps: StepExecution[]; compact?: boolean }) {
  return <div className="relative space-y-0">
    ...
    {steps.map((step, index) => {
      ...
      const isActive = ["RUNNING", "RUNNING_STEP", "COMPENSATING", "COMPENSATING_STEP", "RETRYING"].includes(step.status);
      const isWaiting = ["WAITING", "OPEN"].includes(step.status);
      ...
      return <div className="relative z-10 flex gap-3 py-2.5" key={step.id}>
        <div className={`fg-workflow-node mt-0.5 flex size-[34px] shrink-0 items-center justify-center rounded-full ${isActive ? "fg-workflow-node-active" : ""} ${isWaiting ? "fg-workflow-node-waiting" : ""} ${tone}`}>{iconFor(step.status)}</div>
