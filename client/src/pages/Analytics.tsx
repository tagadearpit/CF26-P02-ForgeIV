...
import { Activity, ArrowUpRight, BarChart3, CheckCircle2, CircleAlert, RefreshCcw, ShieldCheck } from "lucide-react";
...
export default function Analytics() {
  const { auth } = useAuth(); const [executions, setExecutions] = useState<WorkflowExecution[]>([]); const [loading, setLoading] = useState(true); const [refreshing, setRefreshing] = useState(false); const [error, setError] = useState("");
  ...
  const metrics = useMemo(() => {
    ...
    const recovered = executions.filter(item => item.status === "COMPENSATED").length;
    const manual = executions.filter(item => item.status === "MANUAL_RECOVERY_REQUIRED").length;
    const active = executions.filter(item => ["RUNNING", "WAITING_FOR_APPROVAL", "COMPENSATING"].includes(item.status)).length;
  ...
  const stateData = [
    ...
    { name: "Waiting", total: executions.filter(item => item.status === "WAITING_FOR_APPROVAL").length, fill: "#F59E0B" },
    { name: "Compensating", total: executions.filter(item => item.status === "COMPENSATING").length, fill: "#7C5CE7" },
    { name: "Manual", total: metrics.manual, fill: "#E25555" },
  ];
  return <AppShell><PageHeader eyebrow="RELIABILITY ANALYTICS" title="Turn workflow history into operational evidence." description="Outcome rates are calculated from the live durable execution ledger—not from mocked chart values. Use this view to explain what completed, what was safely recovered, and what still needs a person." action={<Button onClick={() => void load(true)} variant="outline" className="border-slate-200"><RefreshCcw className={cn("mr-2 size-3.5", refreshing && "animate-spin")} />Refresh evidence</Button>} />
    ...
    {loading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div className="h-40 animate-pulse rounded-2xl bg-slate-200/70" key={index} />)}</div> : <><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Completion rate" value={`${metrics.successRate}%`} description={`${metrics.completed} workflow${metrics.completed === 1 ? "" : "s"} reached their intended end state.`} icon={CheckCircle2} tone="bg-emerald-50 text-emerald-600" /><Metric label="Safe recovery rate" value={`${metrics.recoveryRate}%`} description={`${metrics.recovered} rejected or failed workflow${metrics.recovered === 1 ? "" : "s"} were compensated.`} icon={ShieldCheck} tone="bg-violet-50 text-violet-600" /><Metric label="Manual exceptions" value={String(metrics.manual)} description="Cases where automation stopped rather than hiding uncertainty." icon={CircleAlert} tone="bg-rose-50 text-rose-600" /><Metric label="Currently active" value={String(metrics.active)} description="Running, waiting, or actively compensating right now." icon={Activity} tone="bg-blue-50 text-blue-600" /></section>
